...
const { kakao } = window;
// Render My Markers
export default function RenderMyMarkers(userInfo) {
    $('.filter-marker li button').removeClass('on');
    $('.filter-marker li').eq(1).children('button').addClass('on');
    ...
    if (userInfo && userInfo.isSuperuser) {
        console.log("관리자");
    } else if (userInfo && userInfo.isStaff) {
        console.log("공인중개사회원");
    } else {
        console.log("일반회원");
    }
    ...
    markers.then((item) => {
        item.forEach((el) => {
            if (el.registrant === userInfo.email) {
                // 주소로 좌표를 검색합니다
